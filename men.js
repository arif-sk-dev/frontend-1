...
const addDataToHTML = (productsToDisplay, append = false) => {
    ...
    showMoreBtn.style.display = displayedProductsCount >= productsToDisplay.length ? "none" : "block";
...
const initApp = () => {
    fetch("products.json")
        ...
        .then(data => {
            ...
            const reversedMenProducts = [...menProducts].reverse();
            currentProducts = reversedMenProducts;
            addDataToHTML(reversedMenProducts);
        });
};
initApp();
...
const showAllMenProducts = () => {
    const menProducts = allProducts.filter(product => product.category === "men");
    const reversed = [...menProducts].reverse();
